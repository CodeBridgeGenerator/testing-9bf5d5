...
const TrainingProviderSInformationFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = trainingProviderSInformationFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("trainingProviderSInformation").create(elem));
...
export default connect(mapState, mapDispatch)(TrainingProviderSInformationFakerDialogComponent);
